...
# Initialize momentums with zeros
beta_m = 0.9
momentums = [torch.zeros_like(p) for p in model.parameters()]

# Initialize adaptive learning rates with zeros
beta_g = 0.999
ε = 1e-8
sq_grads = [torch.zeros_like(p) for p in model.parameters()]
...
#                 param -= learning_rate * param.grad / (torch.sqrt(sq_grad) + ε)
...
#                 # \hat g_{t+1}^2 := \frac{g_{t+1}^2}{1 - β_g^t}
#                 # θ_{t+1} := θ_t - η \frac{\hat m_{t+1}}{\sqrt{\hat g_{t+1}^2} + ε}
#             for param, momentum, sq_grad in zip(
#                 model.parameters(), momentums, sq_grads
#             ):
#                 momentum.set_(beta_m * momentum + (1 - beta_m) * param.grad)
#                 sq_grad.set_(beta_g * sq_grad + (1 - beta_g) * param.grad ** 2)
...
#                 ghat = sq_grad / (1 - beta_g ** t)

#                 param -= learning_rate * mhat / (torch.sqrt(ghat) + ε)
